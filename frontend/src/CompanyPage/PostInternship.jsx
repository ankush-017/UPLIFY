...
function PostInternship() {
    ...
    useEffect(() => {
        // Ensure both VANTA and FOG are available
    ...
    return (
        <div ref={vantaRef} className={`${darkMode ? 'bg-black' : 'bg-blue-50'} min-h-screen flex flex-col justify-center md:pt-8 pt-10 items-center px-4 pb-20`}>
            <h2 className={`text-2xl md:text-4xl font-extrabold text-center mb-8 px-6 py-4 rounded-xl shadow-md backdrop-blur-md bg-white/20 dark:bg-black/30
  ${darkMode ? 'text-white' : 'text-blue-400'}`}>
                🚀 Post an Exciting Internship Opportunity
            </h2>
            <motion.div
      ...
      ${darkMode ? 'bg-black/60 border-blue-700 text-white' : 'bg-white backdrop-blur-md border-blue-700 text-gray-900'}`}
            >
                ...
                <div className="w-full md:w-1/2 p-8">
                    <h2 className={`text-2xl font-bold mb-6 text-center ${darkMode ? 'text-white' : 'text-black'}`}>
                        Post Internship - Uplify
                    </h2>

                    <form onSubmit={handleSubmit} className="space-y-5">
                        {['title', 'company', 'location', 'stipend', 'link'].map((field) => (
                            <input
                                key={field}
                                type="text"
                                name={field}
              ...
              ${darkMode
                  ...
                  ${darkMode ? 'border-white/40' : 'border-gray-500'}`}
                                />
                            ...
                            <label className="flex items-center gap-2 cursor-pointer">
                                <input
                                    ...
                                    onChange={handleChange}
                                    className={`appearance-none w-5 h-5 border rounded-full 
                  checked:border-yellow-500 checked:bg-yellow-400 
                  ${darkMode ? 'border-white/40' : 'border-gray-500'}`}
                                />
                                <span className={`${darkMode ? 'text-white' : 'text-gray-800'} text-sm`}>
                                    Forwarded Source
