...
function AndroidDeveloper() {
  ...
  const prompt = `
Give me a step-by-step tech-stack-name roadmap to become a Android Developer. Return it strictly as a pure JSON array of objects. 
    ...
    <div className={`min-h-screen px-4 py-12 transition-all duration-300 ${darkMode ? "bg-black" : "bg-gradient-to-br from-white via-blue-50 to-blue-100"}`}>
      <div className="max-w-7xl mx-auto">
        <h1 className={`text-4xl md:text-5xl font-extrabold text-center mb-4 ${darkMode ? "text-blue-500" : "text-blue-700"}`}>
          <span className={`${darkMode ? "text-orange-500" : "text-orange-600"}`}>Android Developer</span> Roadmap
        </h1>
        <p className={`text-center mb-12 text-lg max-w-3xl mx-auto ${darkMode ? "text-gray-300" : "text-gray-600"}`}>
         A carefully crafted step-by-step journey to becoming a proficient Android Developer. Dive into Java and Kotlin, master Android Studio, explore app architecture and UI/UX principles, and access top learning resources to build modern, high-performance mobile applications.
        </p>

        {loading ? (
          <div className="flex flex-col items-center justify-center min-h-[200px]">
            <p className="text-lg text-gray-500 text-center mb-4">Preparing Roadmap for Android Developer...</p>
            <Spin size="large" />
          </div>
        ) : error ? (
