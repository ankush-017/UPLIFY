...
function Backend() {
  ...
  const prompt = `
Give me a step-by-step tech stack name roadmap to become a Backend Developer. Return it strictly as a pure JSON array of objects. 
    ...
    <div className={`min-h-screen px-4 py-12 transition-all duration-300 ${darkMode ? "bg-black" : "bg-gradient-to-br from-white via-blue-50 to-blue-100"}`}>
      <div className="max-w-7xl mx-auto">
        <h1 className={`text-4xl md:text-5xl font-extrabold text-center mb-4 ${darkMode ? "text-blue-500" : "text-blue-700"}`}>
          <span className={`${darkMode ? "text-red-500" : "text-red-600"}`}>Backend Developer</span> Roadmap
        </h1>
        <p className={`text-center mb-12 text-lg max-w-2xl mx-auto ${darkMode ? "text-gray-300" : "text-gray-600"}`}>
          A carefully crafted step-by-step journey to becoming a proficient Backend Developer. Explore topics, master skills, and access the best learning resources.
        </p>

        {loading ? (
          <div className="flex flex-col items-center justify-center min-h-[200px]">
            <p className="text-lg text-gray-500 text-center mb-4">Preparing Roadmap for Backend Developer...</p>
            <Spin size="large" />
          </div>
        ) : error ? (
