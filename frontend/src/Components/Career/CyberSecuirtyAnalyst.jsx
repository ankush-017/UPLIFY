...
function CyberSecuirtyAnalyst() {
  ...
  const prompt = `
Give me a step-by-step tech-stack-name roadmap to become a CyberSecuirty Analyst. Return it strictly as a pure JSON array of objects. 
    ...
    <div className={`min-h-screen px-4 py-12 transition-all duration-300 ${darkMode ? "bg-black" : "bg-gradient-to-br from-white via-blue-50 to-blue-100"}`}>
      <div className="max-w-7xl mx-auto">
        <h1 className={`text-4xl md:text-5xl font-extrabold text-center mb-4 ${darkMode ? "text-blue-500" : "text-blue-700"}`}>
          <span className={`${darkMode ? "text-yellow-500" : "text-yellow-600"}`}>CyberSecuirty Analyst</span> Roadmap
        </h1>
        <p className={`text-center mb-12 text-lg max-w-4xl mx-auto ${darkMode ? "text-gray-300" : "text-gray-600"}`}>
           A carefully crafted step-by-step journey to becoming a proficient Cybersecurity Analyst. Explore network security, threat detection, ethical hacking, and incident response. Master tools like Wireshark, Metasploit, and SIEM systems, and access top resources to protect digital assets and ensure organizational security.
        </p>

        {loading ? (
          <div className="flex flex-col items-center justify-center min-h-[200px]">
            <p className="text-lg text-gray-500 text-center mb-4">Preparing Roadmap for CyberSecuirty Analyst ...</p>
            <Spin size="large" />
          </div>
        ) : error ? (
...
export default CyberSecuirtyAnalyst;